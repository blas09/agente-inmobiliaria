...
export async function getTenantById(tenantId: string): Promise<Tables<"tenants">> {
	const supabase = await createSupabaseServerClient();
	const { data, error } = await supabase
		.from("tenants")
		.select("*")
...
export async function getPlatformSummary() {
	...
	const [tenants, activeTenants, memberships] = await Promise.all([
		...
		supabase.from("tenant_users").select("*", { head: true, count: "exact" }).eq("status", "active"),
	]);
...
export async function getAssignableTenantUsers(tenantId: string) {
	...
	return users.filter(
		(user) =>
			user.status === "active" &&
			["tenant_owner", "tenant_admin", "advisor", "operator"].includes(user.role),
	);
}
