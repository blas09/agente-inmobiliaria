...
export async function listProperties(tenantId: string, filters?: { q?: string; status?: string }) {
	...
	if (filters?.q) {
		query = query.or(`title.ilike.%${filters.q}%,city.ilike.%${filters.q}%,neighborhood.ilike.%${filters.q}%`);
	}

	if (filters?.status && propertyStatuses.has(filters.status as PropertyStatus)) {
