...
export async function listLeads(tenantId: string, filters?: { q?: string; status?: string }) {
	...
	if (filters?.status && leadStatuses.has(filters.status as LeadQualificationStatus)) {
		query = query.eq("qualification_status", filters.status as LeadQualificationStatus);
	}
...
export async function getLeadById(tenantId: string, leadId: string): Promise<Tables<"leads">> {
	const supabase = await createSupabaseServerClient();
