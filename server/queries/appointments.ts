...
async function enrichAppointments(appointments: Tables<"appointments">[]) {
	...
	const leadIds = Array.from(new Set(appointments.map((appointment) => appointment.lead_id)));
	...
	const [leadsResponse, propertiesResponse, advisorsResponse] = await Promise.all([
		supabase.from("leads").select("id, full_name, phone, email").in("id", leadIds),
		propertyIds.length > 0
			? supabase.from("properties").select("id, title, external_ref").in("id", propertyIds)
			: Promise.resolve({ data: [], error: null }),
		advisorIds.length > 0
			? supabase.from("user_profiles").select("id, full_name, email").in("id", advisorIds)
			: Promise.resolve({ data: [], error: null }),
	]);

	if (leadsResponse.error) {
		throw new Error(`Failed to load appointment leads: ${leadsResponse.error.message}`);
	}

	if (propertiesResponse.error) {
		throw new Error(`Failed to load appointment properties: ${propertiesResponse.error.message}`);
	}

	if (advisorsResponse.error) {
		throw new Error(`Failed to load appointment advisors: ${advisorsResponse.error.message}`);
	}

	const leadMap = new Map((leadsResponse.data ?? []).map((lead) => [lead.id, lead]));
	const propertyMap = new Map((propertiesResponse.data ?? []).map((property) => [property.id, property]));
	const advisorMap = new Map((advisorsResponse.data ?? []).map((advisor) => [advisor.id, advisor]));

	return appointments.map((appointment) => ({
		...appointment,
		lead: leadMap.get(appointment.lead_id) ?? null,
		property: appointment.property_id ? propertyMap.get(appointment.property_id) ?? null : null,
		advisor: appointment.advisor_id ? advisorMap.get(appointment.advisor_id) ?? null : null,
	}));
}
...
export async function listAppointments(
	...
	if (filters?.status && filters.status !== "all" && appointmentStatuses.has(filters.status as AppointmentStatus)) {
