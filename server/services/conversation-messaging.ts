...
export async function sendManualConversationReply(input: {
	...
	const { data: conversation, error: conversationError } = await supabase
		...
		.single();
	const resolvedConversation = conversation as unknown as OutboundConversationRecord | null;
	...
	if (resolvedConversation.channels?.type !== "whatsapp") {
		throw new Error("Manual outbound is only implemented for WhatsApp channels.");
	}
	...
	if (whatsappAccountError) {
		throw new Error(`Failed to resolve WhatsApp account: ${whatsappAccountError.message}`);
	}
	...
	if (pendingInsert.error || !pendingInsert.data) {
		throw new Error(pendingInsert.error?.message ?? "Failed to create pending outbound message.");
	}
	...
	try {
		...
		const errorMessage = error instanceof Error ? error.message : "Outbound WhatsApp failed.";
