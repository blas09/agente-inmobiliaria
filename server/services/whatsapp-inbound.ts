...
type ProcessingResult =
	| { kind: "message"; status: "processed" | "duplicate" | "ignored"; detail: string }
	| { kind: "status"; status: "processed" | "duplicate" | "ignored"; detail: string };
...
async function createLeadFromWhatsApp(input: {
	...
	if (error || !data) {
		throw new Error(`Failed to create lead from WhatsApp: ${error?.message ?? "unknown error"}`);
	}
...
async function findOrCreateConversation(input: {
	...
	const admin = createSupabaseAdminClient();
	const { data: existingConversations, error: conversationLookupError } = await admin
		.from("conversations")
	...
	if (conversationLookupError) {
		throw new Error(`Failed to lookup conversation: ${conversationLookupError.message}`);
	}
	...
	if (existingConversation) {
		const nextStatus =
			existingConversation.status === "closed" ? "open" : existingConversation.status;
...
async function processStatusEvent(input: {
	...
	if (input.status.id && input.status.status) {
		...
		if (input.status.status === "delivered" && input.status.timestamp) {
			patch.delivered_at = new Date(Number(input.status.timestamp) * 1000).toISOString();
		}
		if (input.status.status === "read" && input.status.timestamp) {
			patch.read_at = new Date(Number(input.status.timestamp) * 1000).toISOString();
		}
