...
export function LeadForm({ action, initialValues, stageOptions, advisorOptions }: LeadFormProps) {
		...
		<form action={formAction} className="space-y-6">
			<Card>
				<CardContent className="grid gap-4 pt-6 md:grid-cols-2">
					...
					>
						<Input defaultValue={initialValues?.full_name ?? ""} id="full_name" name="full_name" required />
					</FormField>
					<FormField htmlFor="phone" label="Teléfono">
						<Input defaultValue={initialValues?.phone ?? ""} id="phone" name="phone" />
					</FormField>
					...
					<div className="grid gap-3 rounded-lg border border-border bg-slate-50 p-4 md:col-span-2 md:grid-cols-3">
						...
						<label className="flex items-center gap-2 text-sm">
							<input defaultChecked={initialValues?.pets ?? false} name="pets" type="checkbox" />
							Tiene mascotas
						</label>
						<label className="flex items-center gap-2 text-sm">
							<input
								defaultChecked={initialValues?.is_human_handoff_required ?? false}
