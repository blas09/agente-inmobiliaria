...
function parseLeadFormData(formData: FormData) {
	...
	return leadSchema.safeParse({
		...
		desired_neighborhood: toNullableString(formData.get("desired_neighborhood")) ?? undefined,
		bedrooms_needed: toNullableInteger(formData.get("bedrooms_needed")),
		financing_needed:
			formData.get("financing_needed") === null ? null : toBoolean(formData.get("financing_needed")),
		pets: formData.get("pets") === null ? null : toBoolean(formData.get("pets")),
		...
		is_human_handoff_required: toBoolean(formData.get("is_human_handoff_required")),
	});
}
...
export async function updateLeadRoutingAction(
	...
	const isHumanHandoffRequired = toBoolean(formData.get("is_human_handoff_required"));

	if (!qualificationStatus || !leadQualificationStatuses.has(qualificationStatus)) {
		return {
			status: "error",
...
export async function deleteLeadAction(leadId: string) {
	...
	await supabase.from("leads").delete().eq("tenant_id", activeTenant.id).eq("id", leadId);
