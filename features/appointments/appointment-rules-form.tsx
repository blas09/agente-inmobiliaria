  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      >
        <div className="flex flex-wrap gap-3">
          {workingDayOptions.map((option) => (
            <label
              key={option.value}
              className="border-border bg-card flex items-center gap-2 rounded-full border px-3 py-2 text-sm"
            >
              <input
                defaultChecked={initialValues.working_days.includes(
                  option.value,
                )}
                name="working_days"
                type="checkbox"
      ...
      <div className="flex justify-end">
        <SubmitButton
          label="Guardar reglas de agenda"
