...
function parseAppointmentFormData(formData: FormData, timeZone: string) {
  ...
  const scheduledAt = zonedDateTimeLocalToUtcIso(
...
function parseAppointmentRulesFormData(formData: FormData) {
  return appointmentRulesSchema.safeParse({
    ...
    business_hours_start:
      formData.get("business_hours_start")?.toString() ?? "",
...
function revalidateAppointmentPaths(paths: string[]) {
  const uniquePaths = Array.from(
    new Set(["/dashboard/appointments", ...paths]),
  );
...
export async function createAppointmentAction(
  ...
  if (
    requiresPlannedScheduleValidation(result.data.status as AppointmentStatus)
  ) {
    ...
    const minimumAllowedDate = new Date(
      Date.now() + appointmentRules.advance_notice_hours * 60 * 60 * 1000,
    );
  ...
  if (
    !(await ensureAssignableAdvisor(activeTenant.id, result.data.advisor_id))
  ) {
    return {
      status: "error",
      message:
...
export async function updateAppointmentAction(
  ...
  if (appointmentError || !currentAppointment) {
    return {
      status: "error",
      message:
        appointmentError?.message ?? "La visita ya no existe en este tenant.",
    };
  }
...
export async function updateAppointmentRulesAction(
  ...
  const { error } = await supabase
    .from("tenants")
    .update({
      settings: mergeAppointmentRulesIntoSettings(
